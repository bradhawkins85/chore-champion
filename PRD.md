...
### Points History View
- **Functionality**: Display a comprehensive timeline of all points earned, expired, spent, swapped, and bonus awards for each child. Shows detailed history with filtering by category and time period (today, week, month, all time). Visualizes category summaries showing earned vs. lost points with net totals. Each event in the timeline shows the description, category, timestamp, and point value with appropriate icons and colors.
- **Purpose**: Provide transparency and insights into point flow over time. Help children and parents understand how points are accumulated and where they go. Demonstrate the impact of point expiry rules and track bonus achievements. Enable accountability and pattern recognition in chore completion and reward spending.
- **Trigger**: Child view → Click "History" button next to Shop button
- **Progression**: Navigate to child's chore view → Click "History" button → View category summary cards showing earned/lost/net points for the selected time period → Filter by time period (All Time, Today, This Week, This Month) → Filter by specific category or view all categories → Scroll through timeline showing chronological events (most recent first) → Each event displays: icon (Star for earned, Sparkle for bonus, Gift for spent, Clock for expired, ArrowsClockwise for swaps), description, category badge with color, timestamp (formatted as "Today 3:45 PM", "Yesterday 5:20 PM", or "Jan 15, 2:30 PM"), and point change (+ for gains, - for losses) → Expired points show the original earn date in description → Click back to return to chore view
- **Success criteria**: History view displays all point events accurately with correct timestamps and point amounts, category filtering shows only events for selected category, time filtering correctly limits events to the selected period, category summary cards calculate totals correctly (earned = chore completions + bonuses + swaps in, lost = spent + expired + swaps out, net = earned - lost), expired points show both the expiry timestamp and original earning date, timeline is responsive and scrollable, empty states show helpful messages when no activity found, visual design uses category colors for badges and appropriate icons for event types, timestamps are human-readable and relative when recent.

### Weather Display
- **Functionality**: Display current weather conditions, temperature, and child-friendly temperature feeling (Freezing, Cold, Cool, Nice, Warm, Hot, Extra Hot) on the main child selector screen. Parents configure the location via geocoding search in the Security settings tab. Temperature unit is automatically detected based on location (Celsius for most countries, Fahrenheit for US, Liberia, Myanmar) with optional manual override. Weather data is fetched from Open-Meteo API and updates every 15 minutes.
- **Purpose**: Help children understand outdoor conditions and dress appropriately. Provide context for the day and create a more engaging, connected experience. Add visual interest to the main screen with weather emojis and color-coded temperature indicators.
- **Trigger**: Automatic display on child selector screen when weather is enabled and location is configured
- **Progression**: Parent enables weather in Security settings → Search and select location via geocoding → Location coordinates are saved → Temperature unit is auto-detected based on country → Optional manual override to Celsius or Fahrenheit → Child selector screen fetches and displays weather → Shows weather emoji, condition, temperature with unit, and child-friendly feeling with emoji → Updates automatically every 15 minutes
- **Success criteria**: Weather displays accurately based on configured location, temperature unit auto-detects correctly for most locations, manual override works reliably, child-friendly temperature feelings use appropriate thresholds (all calculations based on Fahrenheit: ≤32°F = Freezing, ≤45°F = Cold, ≤60°F = Cool, ≤75°F = Nice, ≤85°F = Warm, ≤95°F = Hot, >95°F = Extra Hot), weather updates periodically without user action, geocoding search works for cities, zip codes, and addresses, display shows appropriate weather emojis for conditions (sun, clouds, rain, snow, storm, etc.), graceful handling when weather API is unavailable.

### Seasonal Themes
- **Functionality**: Automatically change the app's color scheme based on current weather conditions when enabled. Different weather states trigger distinct themed color palettes: Snowy (cool blues and whites), Stormy (dark dramatic purples), Rainy (soft gray-blues), Foggy (muted grays), Cloudy (neutral grays), Partly Cloudy (light blues), Clear/Sunny with temperature variations (Extra Hot: warm oranges/reds, Hot: sunny yellows, Warm: bright greens, Nice: fresh greens, Cool: light blues, Cold: icy blues, Freezing: deep blues). Parents can toggle seasonal themes on/off in weather settings. All theme colors use OKLCH color space for consistent perceptual brightness.
- **Purpose**: Create an immersive, dynamic experience that connects the app to the real world. Make the interface feel alive and responsive to environmental conditions. Provide subtle environmental awareness and seasonal ambiance. Enhance engagement through visual variety that changes throughout the day and seasons.
- **Trigger**: Parent Mode → Security → Weather Settings → Enable "Seasonal Themes" toggle → Themes update automatically based on current weather
- **Progression**: Parent enables weather display and configures location → Parent toggles "Seasonal Themes" in weather settings → App fetches current weather data → Theme engine analyzes weather condition code and temperature → Selects appropriate seasonal theme (prioritizes severe conditions like snow/storms, then considers temperature) → Applies theme colors to all CSS custom properties → Theme updates every 15 minutes with weather data → Visual indicator in weather display card shows active theme name → Smooth color transitions between themes → Toast notification informs user when theme changes (only on main screen, not during chore completion)
- **Success criteria**: Themes change accurately based on weather conditions, color transitions are smooth and non-jarring, all UI elements remain readable with proper contrast ratios in all themes, theme persists until weather updates (no flickering), themes apply globally across child and parent modes, default theme is restored when seasonal themes are disabled, theme indicator badge shows in weather display when themes are active, theme changes are logged to console for debugging, OKLCH colors maintain consistent perceived brightness across themes, theme selection logic prioritizes weather conditions correctly (snow > storms > rain > fog > temperature-based themes).
...
### Biometric Authentication & Quick Unlock
- **Functionality**: Optional biometric authentication (fingerprint, Face ID, Touch ID, Windows Hello) as an alternative to PIN for accessing Parent Mode. When enabled on installed PWA, automatically prompts for biometric authentication on app launch for quick access to parent features.
- **Purpose**: Provide faster, more convenient access to Parent Mode while maintaining security. Eliminate the need to remember and type PIN codes. Leverage device biometric capabilities for seamless authentication.
- **Trigger**: Parent Mode → Security tab → Enable biometric authentication → Register device credential → Toggle "Quick Unlock on PWA Launch" for automatic prompt when opening installed app
- **Progression**: Parent enables biometric authentication → Registers device using platform biometric (Face ID, Touch ID, fingerprint, Windows Hello) → Assigns name to credential (e.g., "My iPhone", "Mom's Laptop") → Multiple devices can be registered → When opening Parent Mode, biometric prompt appears alongside or instead of PIN (based on fallback setting) → On installed PWA with quick unlock enabled, biometric prompt automatically appears on app launch → Successful authentication grants immediate parent mode access → Failed attempts fall back to PIN entry → Last used timestamp tracked per credential → Credentials can be individually removed
- **Success criteria**: Biometric registration works on compatible devices (iOS, Android, macOS, Windows with biometric hardware), authentication prompt displays appropriate biometric type for platform (Face ID, Touch ID, fingerprint, Windows Hello), successful biometric auth grants parent mode access without PIN, failed auth allows PIN fallback option, quick unlock automatically triggers on PWA launch when enabled, quick unlock respects device display mode (only on standalone/installed PWA), registered credentials display with names and last used dates, multiple devices can be registered simultaneously, removing all credentials automatically disables biometric auth, authentication respects PIN lockout rules (if account is locked due to failed PIN attempts, biometric is also blocked), credential management available in Security settings, clear indicators show when biometric auth is enabled and available
