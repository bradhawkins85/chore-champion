...
### Device Pinning and Access Control
- **Functionality**: Automatically register each device that accesses the app. Parents can manage device settings from a dedicated Devices tab in parent mode, controlling which children profiles appear on each device and whether parent mode is enabled on that device. Each device is fingerprinted based on browser characteristics and tracked with last-seen timestamp. Devices can be renamed, configured, or deleted.
- **Purpose**: Enable multi-device family setups where different devices serve different purposes - for example, a child's tablet only shows their profile and blocks parent mode, while the main family tablet shows all children and allows parent access. Provides security by limiting parent mode to trusted devices and prevents unauthorized configuration changes.
- **Trigger**: Device registers automatically on first access → Parent mode → Devices tab → Select device → Configure allowed children and parent mode access
- **Progression**: User accesses app from new device → Device automatically registered with default name "Device N" → All children enabled by default, parent mode enabled → Parent navigates to Devices tab → Views list of all registered devices with last-seen timestamps → Click edit on device → Rename device → Toggle parent mode enabled/disabled → Select which children profiles appear on this device → Save changes → Device immediately respects new configuration → When device attempts parent mode while disabled, shows error message → Child selector only shows allowed children for current device → Parent can delete old/unused devices
- **Success criteria**: Devices auto-register on first access with unique fingerprints, device list shows all registered devices with last-seen timestamps and "This Device" badge, device configuration persists and applies immediately, parent mode button blocked with helpful error when disabled on device, child selector filters children based on device allowlist, empty state shown when no children allowed on device with instructions, new children automatically added to all devices' allowlists, deleted children removed from all device allowlists, device names editable with validation, device deletion works with confirmation dialog, current device cannot be deleted, fingerprinting consistent across sessions on same device.
